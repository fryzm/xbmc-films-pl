...
class YouTubePlayer():
    ...
    def downloadSubtitle(self, video={}):
        ...
        if self.settings.getSetting("lang_code") != "0":
            ...
            if subtitle_url:
                ...
                if xml["status"] == 200 and xml["content"]:
                    result += self.transformSubtitleXMLtoSRT(xml["content"].decode('utf-8'))
    ...
    def getSubtitleFileName(self, video):
        get = video.get
        lang_code = ["off", "en", "es", "de", "fr", "it", "ja"][int(self.settings.getSetting("lang_code"))]
        filename = ''.join(c for c in video['Title'] if c not in self.utils.INVALID_CHARS) + "-[" + get('videoid') + "]-" + lang_code.upper() + ".ssa"
                ...
                if stype == "highlight":
                    ...
                    if len(text):
                        ...
                        for snode in self.common.parseDOM(node, "appearance", attrs={"fgColor": ".*?"}, ret=True):
                            ...
                            ns_bcolor = self.transformColor(ns_bcolor[0]) if len(ns_bcolor) > 0 else ""

                            ns_alpha = self.common.parseDOM(snode, "appearance", ret="bgAlpha")
                            ns_alpha = self.transformAlpha(ns_alpha[0]) if len(ns_alpha) > 0 else "-1"
    ...
    def getVideoUrlMap(self, pl_obj, video={}):
        ...
        elif html.find(",itag=") > -1:
            fmt_url_map = html.split(',itag=')
        ...
        if len(fmt_url_map) > 0:
            for index, fmt_url in enumerate(fmt_url_map):
                ...
                if (len(fmt_url) > 7 and fmt_url.find("&") > 7):
                    ...
                    final_url = final_url.replace('sig=', 'signature=')
    ...
    def getLocalFileSource(self, get, status, video):
        ...
        if (get("action", "") != "download"):
            ...
            filename = ''.join(c for c in video['Title'] if c not in self.utils.INVALID_CHARS) + "-[" + get('videoid') + "]" + ".mp4"
