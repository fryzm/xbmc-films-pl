...
# read settings
__addon__ = xbmcaddon.Addon("script.xbmcfilm.com")
...
class xbmcfilmAPI(object):
    ...
    __apikey = "ad34487902838730321da2"
    __baseURL = "http://api.xbmcfilm.com"
    __timeout = 60
    __username = ""
    __password = ""
    validUser = False
    ...
    def watchingMovie(self, imdb_id, title, year, duration, percent,file='',image='',sessionid='',xbmcpath=''):
        data = {'imdb_id': imdb_id, 'title': title, 'year': year, 'duration': math.ceil(duration), 'progress': math.ceil(percent),'file':file,'image':image, 'id_session':sessionid, 'xbmcpath': xbmcpath}
    ...
    def scrobbleMovie(self, imdb_id, title, year, duration, percent,file='',image='',sessionid='',xbmcpath=''):
        data = {'imdb_id': imdb_id, 'title': title, 'year': year, 'duration': math.ceil(duration), 'progress': math.ceil(percent),'file': file,'image': image, 'id_session':sessionid,'xbmcpath': xbmcpath}
