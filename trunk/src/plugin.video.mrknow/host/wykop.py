...
class WYKOP:
    ...
    def listsItems(self, url):
        ...
        match = re.compile('<strong>\s(.*?)<a href="(.*?)">(.*?)</a>(.*?)</strong>', re.DOTALL).findall(readURL)
       ...
       #         print match1
       #         if len(match1) > 0:
       #             self.add('wykop', 'playSelectedMovie', 'None', match[i][3], match[a][1], match[a][0], match[a][4], 'None', True, False)
    ...
    def getMovieLinkFromXML(self, url):
        ...
        readURL = openURL.read()
        openURL.close()
        match = re.compile('<blockquote cite="(.*?)"', re.DOTALL).findall(readURL)
        print "Film"
        print match
